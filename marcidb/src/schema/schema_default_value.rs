use crate::{Field, schema::{FieldType, PrimitiveFieldType, schema_attributes::Attribute}};
...
pub fn resolve_default_value(field: &mut Field) {    
        ...
        match &field.ty {
            FieldType::Enum(enum_def) => {
                let Some(val) = enum_def.variants_map.get(default_str) else {
                    panic!("Cannot find default value {} in enum. Field: {}", default_str, field.full_name)
                };
            ...
            FieldType::RefList(_) | FieldType::RefListUnresolved(_) => {
                if default_str == "[]" {
                    continue;
                } else {
                    panic!("Default value for field {} is not supported", field.full_name);
                }
            },
...
fn parse_default_value(ty: &PrimitiveFieldType, value: &str, field: &Field) -> FieldDefault {
    if value == "autoincrement()" {
        return FieldDefault::Counter(0)
    }

    match ty {
        PrimitiveFieldType::String => FieldDefault::Value(value.trim().trim_matches('"').as_bytes().to_vec()),
        PrimitiveFieldType::Int64 => FieldDefault::Value(value.parse::<i64>().unwrap().to_be_bytes().to_vec()),
        ...
        PrimitiveFieldType::Bool => {
            match value.to_lowercase().as_str() {
                ...
                _ => panic!("Failed to parse default value {}. Field: {}", value, field.full_name)
            }
        },
...
fn parse_default_value_list(ty: &PrimitiveFieldType, value: &str, fixed_size: Option<usize>, field: &Field) -> FieldDefault {
    let Some(value) = value.strip_prefix('[').and_then(|f| f.strip_suffix(']')) else {
        panic!("Failed to parse list default value {}. Field: {}", value, field.full_name);
    };
    ...
    if let Some(fixed_size) = fixed_size && count != fixed_size {
        panic!("Failed to parse default value: Wrong size, expected: {}. Field: {}", fixed_size, field.full_name)
    }
    ...
    for item in splited {
        let FieldDefault::Value(binary) = parse_default_value(ty, item, field) else {
            panic!("Failed to parse default value {}. Field: {}", item, field.full_name)
        };
        if is_dynamic_items {
